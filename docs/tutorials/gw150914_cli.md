...
`config.final.toml` records the exact resolved configuration (including any defaults that were applied). To reproduce the run, pass it directly to `jim-run`. Because `config.final.toml` retains the original `output.dir`, the run will abort if that directory already exists. Before re-running, either change `output.dir` to a new path or add `output.overwrite = true`:
